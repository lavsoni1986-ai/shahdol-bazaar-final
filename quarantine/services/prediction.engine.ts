...
export const analyzeEventPatterns = async (districtId: number) => {
  ...
  const patterns = {
    searchTrends: events.filter(e => e.type.includes('search')).length,
    clickTrends: events.filter(e => e.type.includes('click')).length,
    orderTrends: events.filter(e => e.type.includes('order')).length,
    userActivity: events.reduce((acc, e) => {
      acc[e.userId || 'anonymous'] = (acc[e.userId || 'anonymous'] || 0) + 1;
      return acc;
...
export const predictMarketTrends = async (districtId: number) => {
  ...
  const categorySearches = events
    .filter(e => e.type.includes('search') && e.metadata && typeof e.metadata === 'object' && 'intent' in e.metadata && e.metadata.intent === 'category')
    .reduce((acc, e) => {
      const category = (e.metadata && typeof e.metadata === 'object' && 'category' in e.metadata && typeof e.metadata.category === 'string') ? e.metadata.category : 'unknown';
