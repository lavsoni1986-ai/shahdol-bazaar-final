...
export default function MerchantStoreSetup() {
  ...
  const { data: existingShop } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", "/marketplace/vendors/mine");
  ...
  const { data: districts } = useQuery({
    queryKey: ["districts"],
    queryFn: async () => {
      const res = await apiRequest("GET", "/marketplace/districts");
      if (!res.ok) return [];
      return res.json();
    },
  });
  ...
  const createShopMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      const res = await apiRequest("POST", "/partner/shop/create-default", {
        name: data.name,
