...
// Business Type from Unified Schema
type BusinessType = 'PRODUCT' | 'SERVICE' | 'HEALTHCARE';

// Map category to businessType based on the new unified schema
const getBusinessType = (category: string): BusinessType => {
  const healthcareCategories = ['Hospital', 'Pharmacy'];
  const serviceCategories = ['Restaurant', 'Plumber', 'Electrician', 'Other', 'School'];
  
  if (healthcareCategories.includes(category)) {
    return 'HEALTHCARE';
  }
  if (serviceCategories.includes(category)) {
    return 'SERVICE';
  }
  return 'PRODUCT'; // Default for product shops (Grocery, Electronics, Clothing, etc.)
};
  ...
  const onSubmitProfile = async (data: ShopProfile) => {
    ...
    try {
      ...
      const response = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({
          role: "seller",
          username: data.shopMobile, // Use mobile as username
          password: data.password,
          shopName: data.shopName,
          shopCategory: data.shopCategory,
          shopMobile: data.shopMobile,
          shopAddress: data.shopAddress,
          // Include businessType based on category mapping
          businessType: getBusinessType(data.shopCategory),
        }),
      });
