...
export function ShahdolAIAssistant() {
  ...
  useEffect(() => {
    // Listen for custom event to open AI modal
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 md:right-10 z-[99999] w-80 md:w-96 animate-in slide-in-from-bottom-4 fade-in duration-300">
          ...
          <div className="backdrop-blur-xl bg-black/60 border border-white/20 rounded-2xl shadow-2xl overflow-hidden">
            {/* Header with gradient */}
            <div className="bg-gradient-to-r from-violet-600 via-orange-500 to-purple-600 p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center">
                  <Brain className="text-white w-5 h-5" />
                </div>
                <div>
                  <span className="font-bold text-white">Shahdol AI</span>
            ...
            <div className="h-80 overflow-y-auto p-4 space-y-4 bg-black/20 backdrop-blur-sm">
              {messages.length === 0 && (
                <div className="text-center text-slate-300 py-8">
                  <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-gradient-to-r from-violet-500 to-orange-500 flex items-center justify-center">
                    <Sparkles className="text-white w-8 h-8" />
                  </div>
                  <p className="text-sm font-medium">नमस्ते! मैं Shahdol Bazaar AI हूँ।</p>
                  <p className="text-xs mt-2 text-slate-400">आप मुझसे शहडोल में दुकानों, हॉस्पिटल्स, बसों और products के बारे में पूछ सकते हैं।</p>
                </div>
              )}
              {messages.map((msg, i) => (
                <div 
            ...
            <form onSubmit={handleSubmit} className="p-3 border-t border-white/10 bg-black/30 backdrop-blur-sm flex gap-2">
              ...
              <Button 
                ...
                className="bg-gradient-to-r from-violet-600 to-orange-500 hover:from-violet-500 hover:to-orange-400 border-0"
              >
                <Send size={18} />
              </Button>
            </form>
...
export default ShahdolAIAssistant;
