...
export async function applyFraudActions(vendorId: number) {
  ...
  try {
    const vendor = await prisma.vendor.findUnique({
      where: { id: vendorId },
      select: { districtId: true, fraudScore: true, dsslScore: true }
    });
  ...
  if (finalScore > restrictThreshold) {
    const recentAlert = await prisma.adminLog.findFirst({
      where: {
        action: "FRAUD_ALERT",
        targetId: vendorId,
    ...
    if (!recentAlert) {
      await prisma.adminLog.create({
        data: {
          ...
          action: "FRAUD_ALERT",
          targetId: vendorId,
          details: `Fraud alert: Score ${finalScore}, Status: ${status}`,
          meta: { fraudScore: finalScore, reasons: context.reasons }
        }
      });
